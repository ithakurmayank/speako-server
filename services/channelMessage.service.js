...
const editChannelMessage = async ({
  ...
  // Step 5: Validate mentioned users
  ...
  // Step 6: Fetch files to be removed so we can soft-delete them
  ...
  // Step 7: Persist all changes atomically
  // All writes happen in a single transaction:
  ...
  try {
    await session.withTransaction(async () => {
      // 7a — Soft-delete files being removed from the message
      if (fileIdsToRemove.length > 0) {
        await File.updateMany(
          { _id: { $in: fileIdsToRemove } },
          {
      ...
      // 7b — Mark new files as attached and scope them to this channel
      if (fileIdsToAdd.length > 0) {
        await File.updateMany(
          { _id: { $in: fileIdsToAdd } },
          {
      ...
      // 7c-7e — Update message: attachments (ordered), mentions, edit metadata
      ...
      // 7f — Queue Cloudinary deletes for removed files inside the transaction
  ...
  // Step 8: Return the updated message shaped as MessageDto
  return await getMessageById(messageId, userId);
};

//#endregion

// #region Shared Helpers: fetch a single message and shape it as MessageDto
