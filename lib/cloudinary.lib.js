...
const orgIconFolder = (orgId) => `orgs/${orgId}`;

const teamIconFolder = (orgId, teamId) => `orgs/${orgId}/teams/${teamId}`;

const groupIconFolder = (orgId, conversationId) =>
  `${orgId}/conversations/${conversationId}`;

const channelAttachmentFolder = (orgId, teamId, channelId, date) =>
  `orgs/${orgId}/teams/${teamId}/channels/${channelId}/${getMonthPartition(date)}`;

const conversationAttachmentFolder = (conversationId, date) =>
  `conversations/${conversationId}/${getMonthPartition(date)}`;

const orgRootPrefix = (orgId) => `orgs/${orgId}/`;
...
const uploadAttachment = async (buffer, mimeType, context) => {
  const { orgId, teamId, channelId, conversationId, date } = context;

  if (!(teamId && channelId) && !conversationId) {
    throw new ErrorHandler(
      "Provide (teamId & channelId) OR conversationId",
  ...
  if (!FILE_TYPES_VALUES.includes(fileType)) {
    throw new ErrorHandler(
      `File type '${fileType}' is not allowed for attachments.`,
      EXCEPTION_CODES.INVALID_FILE_TYPE,
    );
  }

  const folder = channelId
    ? channelAttachmentFolder(orgId, teamId, channelId, date)
    : conversationAttachmentFolder(conversationId, date);
...
const uploadIcon = async (buffer, entityType, ids) => {
  ...
  const result = await uploadBufferToCloudinary(buffer, {
    folder,
    public_id: "icon", //use a constant public_id to overwrite the same file everytime on new upload
    overwrite: true,
...
const deleteCloudinaryFile = async (publicId, cloudinaryResourceType) => {
  if (!publicId || !cloudinaryResourceType) {
    throw new ErrorHandler(
      "publicId and cloudinaryResourceType are required",
      EXCEPTION_CODES.MISSING_REQUIRED_FIELDS,
    );
  }
